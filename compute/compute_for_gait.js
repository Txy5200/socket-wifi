...
let gaitNum_L = 0 // 左脚完整步态数
let gaitNum_R = 0 // 右脚完整步态数
...
process.stdin.on('readable', () => {
  let chunk
  while ((chunk = process.stdin.read(56)) !== null) {
    computeGait(chunk)
  }
})
...
const computeCharacteristic = (characteristic, msFromStart) => {
  ...
  if (characteristicList.toString() == [1, 2, 3, 4, 1].toString()) {
    sendfirstGiatSignal()
    let gaitCycle_L = characteristicListTime[4] - characteristicListTime[0]
    let singleStance_L = characteristicListTime[2] - characteristicListTime[1]
    let doubleStance_land_L = characteristicListTime[1] - characteristicListTime[0]
    let doubleStance_leave_L = characteristicListTime[3] - characteristicListTime[2]

    target.gaitCycle_L = (target.gaitCycle_L * gaitNum_L + gaitCycle_L) / (gaitNum_L + 1)
    target.singleStance_L = (target.singleStance_L * gaitNum_L + singleStance_L) / (gaitNum_L + 1)
    target.doubleStance_land_L = (target.doubleStance_land_L * gaitNum_L + doubleStance_land_L) / (gaitNum_L + 1)
    target.doubleStance_leave_L = (target.doubleStance_leave_L * gaitNum_L + doubleStance_leave_L) / (gaitNum_L + 1)
    if (target.gaitCycle_L) target.singleStability_L = target.singleStance_L / target.gaitCycle_L
  ...
  if (characteristicList.toString() == [3, 4, 1, 2, 3].toString()) {
    sendfirstGiatSignal()
    let gaitCycle_R = characteristicListTime[4] - characteristicListTime[0]
    let singleStance_R = characteristicListTime[2] - characteristicListTime[1]
    let doubleStance_land_R = characteristicListTime[1] - characteristicListTime[0]
    let doubleStance_leave_R = characteristicListTime[3] - characteristicListTime[2]

    target.gaitCycle_R = (target.gaitCycle_R * gaitNum_R + gaitCycle_R) / (gaitNum_R + 1)
    target.singleStance_R = (target.singleStance_R * gaitNum_R + singleStance_R) / (gaitNum_R + 1)
    target.doubleStance_land_R = (target.doubleStance_land_R * gaitNum_R + doubleStance_land_R) / (gaitNum_R + 1)
    target.doubleStance_leave_R = (target.doubleStance_leave_R * gaitNum_R + doubleStance_leave_R) / (gaitNum_R + 1)
    if (target.gaitCycle_R) target.singleStability_R = target.singleStance_R / target.gaitCycle_R

    gaitNum_R++
  }

  if (target.singleStance_L + target.singleStance_R > 0) target.singleASI = Math.abs((2 * (target.singleStance_L - target.singleStance_R)) / (target.singleStance_L + target.singleStance_R))
